...
8. `docs/session-ops/handovers/SESSION_04_HANDOVER.md`
...
- `scripts/ralphloop/artifact_io.py`
...
4. backend가 아직 없는 부분은 contract-shaped stub로 막고, 이후 `assistant-api` runtime으로 교체 가능하게 유지한다.
...
4. `docs/session-ops/handovers/SESSION_05_HANDOVER.md`
5. `docs/session-ops/prompts/SESSION_06_PROMPT.md`

작업 규칙:

- `assistant-web`은 `assistant-api` 계약을 바꾸지 않는다. 필요한 mock/stub도 같은 schema shape를 따라야 한다.
